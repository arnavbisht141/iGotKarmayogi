  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl p-0 overflow-hidden bg-slate-50" onClose={onClose}>
        ...
        <div
          ref={printRef}
          className="relative p-10 bg-white border-8 border-slate-900 shadow-2xl m-4 rounded-xl text-center"
        >
          {/* Subtle Guilloche Inner Border */}
          <div className="absolute inset-3 border-2 border-amber-600/40 rounded-lg pointer-events-none" />
          <div className="absolute inset-4 border border-slate-300 rounded-md pointer-events-none" />

          {/* Header & National Emblem Styling */}
          <div className="flex flex-col items-center mb-6">
            <div className="h-14 w-14 rounded-full bg-amber-50 border-2 border-amber-600 flex items-center justify-center mb-3">
              <Award className="h-8 w-8 text-amber-700" />
            </div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500">
              Government of India • Ministry of Personnel, Public Grievances and Pensions
            </h4>
            <h2 className="text-2xl font-serif font-bold text-slate-900 mt-1 tracking-tight">
          ...
          {/* Recipient Details */}
          <p className="text-sm text-slate-500 italic mb-2">This is to officially certify that</p>
          <h1 className="text-3xl font-serif font-extrabold text-slate-900 underline decoration-amber-600/60 decoration-2 underline-offset-8 mb-4">
            {certificate.recipient_name}
          </h1>

          <p className="text-sm text-slate-600 max-w-xl mx-auto mb-4 leading-relaxed">
            has successfully completed the prescribed curriculum and demonstrated professional competency in
          </p>

          <div className="bg-slate-50 border border-slate-200 rounded-xl py-3 px-6 max-w-xl mx-auto mb-6">
            <h3 className="text-xl font-bold text-slate-900">{certificate.course_title}</h3>
          ...
          <div className="flex justify-center items-center gap-6 mb-8 text-left">
          ...
          <div className="grid grid-cols-3 items-end pt-6 border-t border-slate-200 text-xs text-slate-600">
            <div>
        ...
        {/* Action Controls */}
        <div className="flex items-center justify-end gap-3 px-8 py-4 bg-white border-t border-slate-200">
          <Button variant="outline" size="sm" onClick={handlePrint} className="gap-2">
            <Printer className="h-4 w-4" /> Print Certificate
          </Button>
          <Button variant="default" size="sm" onClick={handlePrint} className="gap-2 bg-slate-900">
