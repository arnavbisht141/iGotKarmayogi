"""Base classes and interfaces for procedural cybersecurity challenge templates."""
...
class BaseChallengeTemplate(ABC):
    ...
    flag computation, hint generation, and Marimo notebook instantiation.
    """
    ...
    def generate_hints(self, slots: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        These are locked until the officer explicitly unlocks them.
        """
        pass
    ...
    def build_challenge_package(
        ...
        and synchronized to CTFd/LMS.
        """
        chal_id = (
            challenge_id or f"{self.template_id}-{slots.get('seed_hash', 'auto')[:8]}"
        )
        ...
        manifest = {
            "id": chal_id,
            "title": f"{self.title}: {slots.get('incident_codename', 'Operation Sovereign')}",
